...
// NEW: Helper data for seasonal date calculations
...
function App() {
  ...
  // NEW: Smarter function to determine the correct start date for a given season
  const getSuggestedPlantingDate = (plantingSeason) => {
    const now = new Date();
    const currentMonth = now.getMonth();
    const currentYear = now.getFullYear();
    
    // Find the first season listed in the string (e.g., "Spring" from "Spring/Autumn")
    const seasonMatch = plantingSeason.match(/Spring|Summer|Autumn|Winter/i);
    if (!seasonMatch) return now; // Default to today if no season found
    const season = seasonMatch[0];

    const targetMonth = seasonStartMonths[hemisphere][season];
    let targetYear = currentYear;

    // If the target season's start month has already passed this year, aim for next year.
    if (currentMonth > targetMonth) {
      targetYear++;
    }

    return new Date(targetYear, targetMonth, 1); // Start at the beginning of the season's first month
  };
  ...
  const plannedBeds = useMemo(() => {
    ...
    beds.forEach(bed => {
      ...
      bed.plants.forEach(plant => {
        const plantingSeason = plant.planting[climate] || "Spring"; // Default to Spring if not specified
