...
// --- DATA ---
// NOTE: The full plantData array is included below.
...
function App() {
  ...
  const calculateHarvestDate = (plantedDate, daysToHarvest) => {
    ...
    // Handle date ranges in harvest time (e.g., "60-85") by taking the average.
  ...
  const plannedBeds = useMemo(() => {
    ...
    const groups = gardenPlan.reduce((acc, plant) => {
      const key = `${plant.position}|${plant.soil}`; // Group by position and soil
      if (!acc[key]) {
        acc[key] = {
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {bed.plants.map(plant => (
                                            <tr key={plant.name}>
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap text-orange-600 font-semibold">{plant.suggestedDate.toLocaleDateString(undefined, { month: 'long', day: 'numeric' })}</td>
