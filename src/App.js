...
function App() {
  ...
  // REVISED: Simplified PDF handler for single-column layout
  const handleDownloadPdf = () => {
    const input = document.getElementById('garden-plan-section');
    if (!input) return;

    // Temporarily hide remove buttons for PDF
    const removeButtons = input.querySelectorAll('.remove-button');
    removeButtons.forEach(btn => btn.style.display = 'none');
    
    window.html2canvas(input, { scale: 2 })
      .then((canvas) => {
        // Show remove buttons again after canvas is created
  ...
  return (
    <div className="min-h-screen font-sans" style={{backgroundColor: '#F3EAD3'}}>
      ...
      <main className="p-4 md:p-8">
        ...
        {gardenPlan.length > 0 && (
            <section className="bg-white/80 p-6 rounded-xl shadow-md mb-8">
                {/* ID is now on this wrapper for the PDF */}
                <div id="garden-plan-section">
                    ...
                    {/* The title for the PDF version */}
                    <h2 className="text-3xl font-bold text-center mb-6 hidden print:block">Home Harvest - Garden Plan</h2>
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200">
                                        <thead className="bg-green-700 text-white">
                                            <tr>
                                                ...
                                                {/* REVISED: This column is hidden on screen, only visible in the PDF */}
                                                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider w-1/4 hidden print:table-cell">Notes</th>
                                                {/* REVISED: This column is visible on screen, hidden in the PDF */}
                                                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider print:hidden">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {bed.plants.map(plant => (
                                                <tr key={plant.name}>
                                                    ...
                                                    {/* REVISED: Empty cell for notes in the PDF */}
                                                    <td className="hidden print:table-cell"></td>
                                                    {/* REVISED: Remove button is hidden in the PDF */}
