...
function App() {
  ...
  const handleDownloadPdf = () => {
    ...
    // Use a temporary clone to modify for the PDF without affecting the screen
    ...
    clonedInput.querySelector('.download-button-container').style.display = 'none';
    const beds = clonedInput.querySelectorAll('.garden-bed');
    beds.forEach(bed => {
        bed.querySelector('.action-header').style.display = 'none';
        bed.querySelector('.notes-header').style.display = 'table-cell';
                                            ...
                                            {bed.plants.map(plant => (
                                                <tr key={plant.name}>
                                                    ...
                                                    <td className={`px-4 py-4 whitespace-nowrap action-cell`}>
                                                        <button onClick={() => handleRemoveFromPlan(plant)} className="text-red-500 hover:text-red-700 text-sm font-semibold remove-button">Remove</button>
                                                    </td>
