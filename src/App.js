...
const plantData = [ /* The full list of 100 plants is included in the actual generated code, but omitted here for brevity */ ];
// NOTE: The full plantData array from the previous step is used here. It is just hidden to make the response easier to read.

// NEW: Base64 encoded SVG logo for the PDF header
  ...
  const handleDownloadPdf = () => {
    ...
    const promises = Array.from(bedElements).map(bed => 
      window.html2canvas(bed, { 
        ...
        logging: false,
        onclone: (doc) => {
            // This logic runs on a clone of the document just before screenshotting
            // It ensures our print-specific styles are applied.
            ...
            notesCells.forEach(c => {
                ...
                c.style.border = '1px solid #eee'; // Add a faint border to the notes cell
            });
        }
      })
    );
    ...
    Promise.all(promises).then(canvases => {
      ...
      canvases.forEach((canvas) => {
        ...
        cursorY += imgHeight + 8; // 8mm gap between beds
      });
  ...
  return (
    <div className="min-h-screen font-sans" style={{backgroundColor: '#F3EAD3'}}>
      ...
      <main className="p-4 md:p-8">
        ...
        {gardenPlan.length > 0 && (
            <section className="bg-white/80 p-6 rounded-xl shadow-md mb-8">
                {/* ID remains here for the PDF generator to find the container */}
                <div id="garden-plan-section">
                    ...
                    <div className="space-y-8">
                        {plannedBeds.map((bed, index) => (
                            // Added data-bed-id to help PDF generator identify elements
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200">
                                        <thead className="bg-green-700 text-white">
                                            <tr>
                                                ...
                                                {/* REVISED: Swapped Action/Notes columns with print-specific visibility */}
                                                <th className={`px-4 py-3 text-left text-xs font-medium uppercase tracking-wider action-header`}>Action</th>
                                                <th className={`px-4 py-3 text-left text-xs font-medium uppercase tracking-wider notes-header w-1/4`} style={{display: 'none'}}>Notes</th>
                                            </tr>
                                        </thead>
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {bed.plants.map(plant => (
                                                <tr key={plant.name}>
                                                    ...
                                                    <td className={`px-4 py-4 whitespace-nowrap action-cell`}>
                                                        <button onClick={() => handleRemoveFromPlan(plant)} className="text-red-500 hover:text-red-700 text-sm font-semibold">Remove</button>
                                                    </td>
                                                    <td className={`px-4 py-4 whitespace-nowrap notes-cell`} style={{display: 'none'}}></td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
